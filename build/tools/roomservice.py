...
if not depsonly:
    print "Device %s not found. Attempting to retrieve device repository from Pearl-OS Github (http://github.com/PearlOS)." % device
...
def is_in_manifest(projectname, branch):
    try:
        lm = ElementTree.parse(".repo/local_manifests/pearl_manifest.xml")
        lm = lm.getroot()
    except:
        lm = ElementTree.Element("manifest")
...
def add_to_manifest_dependencies(repositories):
    ...
    for repository in repositories:
        ...
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": repo_name, "revision": pearl_branch })
    ...
    f = open('.repo/local_manifests/pearl_manifest.xml', 'w')
    f.write(raw_xml)
    f.close()
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            if existing_project.attrib['revision'] == repository['branch']:
                print 'PearlOS/%s already exists' % (repo_name)
            else:
                print 'updating branch for PearlOS/%s to %s' % (repo_name, repository['branch'])
                existing_project.set('revision', repository['branch'])
            continue

        print 'Adding dependency: PearlOS/%s -> %s' % (repo_name, repo_target)
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "PearlOS/%s" % repo_name, "revision": pearl_branch })
...
def add_gitlab_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "gitlab", "name": repo_name, "revision": pearl_branch })
...
if depsonly:
    ...
    for repository in repositories:
        ...
        if repo_name.startswith("android_device_") and repo_name.endswith("_" + device):
            ...
            add_to_manifest([{'repository':repo_name,'target_path':repo_path,'branch':pearl_branch}])
