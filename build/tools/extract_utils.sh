...
function setup_vendor() {
    ...
    export ANDROIDBP="$PEARL_ROOT"/"$OUTDIR"/Android.bp
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$PEARL_ROOT"/prebuilts/tools-custom/common/smali/baksmali.jar
        export SMALIJAR="$PEARL_ROOT"/prebuilts/tools-custom/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$PEARL_ROOT"/prebuilts/tools-custom/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$PEARL_ROOT"/prebuilts/tools-custom/${HOST}-x86/bin/compact_dex_converter
    fi
        ...
        if [ "$MD5" != "$OLDMD5" ]; then
            ...
            do
                ...
                if [ -a "$DUMPDIR"/"$PARTITION".new.dat ]; then
                    ...
                    python "$PEARL_ROOT"/vendor/pearl/build/tools/sdat2img.py "$DUMPDIR"/"$PARTITION".transfer.list "$DUMPDIR"/"$PARTITION".new.dat "$DUMPDIR"/"$PARTITION".img 2>&1
