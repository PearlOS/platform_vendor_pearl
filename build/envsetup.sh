...
- pixelgerrit:     A Git wrapper that fetches/pushes patch from/to PearlOS Gerrit Review.
- pixelrebase:     Rebase a Gerrit change and push it again.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for PearlOS Github.
...
- repopick:        Utility to fetch changes from PearlOS Gerrit.
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/PearlOS-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.pearl.device | grep -q "$CUSTOM_BUILD"); then
...
function githubremote()
{
    ...
    git remote add github https://github.com/PearlOS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.pearl.device | grep -q "$LINEAGE_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.pearl.device | grep -q "$CUSTOM_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
